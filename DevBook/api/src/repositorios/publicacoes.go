...
//NovoRepositorioDePublicacaoes cria um repositório de publicações
...
// Buscar traz as publicações dos usuários seguidos e do próprio usuário que fez a requisição
...
// Deletar delela uma publicação do banco de dados
...
func (repositorio Publicacoes) BuscarPorUsuario(usuarioID uint64) ([]modelos.Publicacao, error) {
	...
	if linhas.Next() {
		...
		if erro = linhas.Scan( //Scan usando para preencher a publicacao especifica
			&publicacao.ID,
			&publicacao.Titulo,
			&publicacao.Conteudo,
			&publicacao.AutorID,
